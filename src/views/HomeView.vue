<template>
  <div class="container mt-5">
      ...
      <form @submit.prevent="add()" class="row">
        <div class="mb-3 col-6">
          <label for="exampleFormControlInput1" class="form-label"
            ><h5>Ingreso</h5></label
          >
          <select class="form-select" v-model="titulo" required>
    ...
    <div class="box3 mt-3 row card">
      <div class="row" v-for="ingreso in ingresos" :key="ingreso">
...
export default {
  ...
  methods: {
    ...
    delet(i) {
      let filtro = this.ingresos.filter((value) => value !== i);

      this.ingresos = filtro;
      let counter = 0;
      this.ingresos.forEach((i) => {
        counter = counter + i.cantidad;
      });
